		...
		if selected_file.endswith('.csv'):
			...
			try:
				...
				if st.checkbox("Show column info"):
					st.write(pd.DataFrame({"dtype": data.dtypes, "nulls": data.isnull().sum()}))
				if st.checkbox("Show all data (may be slow)"):
					st.dataframe(data)
				if st.checkbox("Show value counts for a column"):
					col = st.selectbox("Select column", data.columns)
					st.write(data[col].value_counts())
			except Exception as e:
				st.error(f"Could not load or display CSV: {e}")
		elif selected_file.endswith('.txt'):
			try:
				with open(file_path, 'r') as f:
					content = f.read()
				st.text_area("File contents", content, height=400)
			except Exception as e:
				st.error(f"Could not load or display TXT: {e}")

elif page == "Model Performance":

	import mlflow
	import pandas as pd
	st.header("Model Performance & MLflow Integration")
	try:
		# Use mlflow.search_registered_models for compatibility
		models = mlflow.search_registered_models()
		if not models:
			st.warning("No models found in MLflow Model Registry.")
		else:
			model_names = [m.name for m in models]
			selected_model = st.selectbox("Select a registered model", model_names)
			client = mlflow.tracking.MlflowClient()
			model_versions = client.get_latest_versions(selected_model)
			if not model_versions:
				st.warning("No versions found for this model.")
			else:
				# Only show available model versions in the dropdown
				version_info = [f"v{v.version} ({v.current_stage})" for v in model_versions]
				version_map = {f"v{v.version} ({v.current_stage})": v for v in model_versions}
				if not version_info:
					st.warning("No available versions for this model.")
				else:
					selected_version = st.selectbox("Select version", version_info)
					ver = version_map[selected_version]
					run_id = ver.run_id
					st.write(f"Model: {selected_model}, Version: {ver.version}, Stage: {ver.current_stage}")
					# Show run metrics and params
					run = client.get_run(run_id)
					st.subheader("Run Metrics")
					st.write(run.data.metrics)
					st.subheader("Run Parameters")
					st.write(run.data.params)
					st.subheader("Run Tags")
					st.write(run.data.tags)
					if st.checkbox("Show full run info"):
						st.write(run)
					# Optionally, show artifacts (if any)
					if st.checkbox("List run artifacts"):
						artifacts = client.list_artifacts(run_id)
						if artifacts:
							st.write([a.path for a in artifacts])
						else:
							st.info("No artifacts found for this run.")
					# Optionally, show model version history
					if st.checkbox("Show model version history"):
						for v in model_versions:
							st.write(f"Version {v.version}: Stage={v.current_stage}, Run ID={v.run_id}")
	except Exception as e:
		st.error(f"Could not load model registry info: {e}")
